...
/** Featured work — big image cards with an inner parallax drift on scroll. */
export default function Work() {
  ...
  useReveal(root, (scroller) => {
    ...
    gsap.utils.toArray<HTMLElement>('.work-card').forEach((card) => {
      ...
      // inner image parallax while the card crosses the viewport
      ...
      <div className="max-w-7xl mx-auto">
        <div className="work-head flex flex-col md:flex-row md:items-end md:justify-between gap-6">
          ...
          <p className="reveal max-w-sm text-white/55">
            Platforms, products and tools we’ve taken from idea to tens of
            thousands of users.
          </p>
        </div>
        ...
        <div className="mt-16 grid gap-6 md:grid-cols-2">
          {projects.map((p, i) => (
            ...
            >
              <div
                className={`relative overflow-hidden ${i % 3 === 0 ? 'aspect-[16/8]' : 'aspect-[16/10]'}`}
              >
                <img
                  src={p.image}
                  alt={p.name}
                ...
                <div className="absolute bottom-0 left-0 right-0 p-6 md:p-8 flex items-end justify-between gap-4">
                  <div>
                    ...
                    <p className="mt-2 text-sm text-white/60 max-w-md hidden sm:block">
                      {p.blurb}
                    </p>
                  </div>
                  <div className="text-right shrink-0">
                    <div className="font-mono text-xs accent whitespace-nowrap">
                      {p.metric}
                    </div>
