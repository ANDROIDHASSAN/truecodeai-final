          ...
          <div className="grid gap-y-12 md:grid-cols-5 md:gap-x-6">
            {process.map((step) => (
              <div key={step.n} className="proc-step relative">
                ...
                <h3 className="mt-5 font-display text-xl md:text-2xl font-medium text-white">{step.title}</h3>
                <p className="mt-3 text-sm text-white/55 leading-relaxed md:pr-4">{step.desc}</p>
              </div>
            ))}
          </div>
        </div>
